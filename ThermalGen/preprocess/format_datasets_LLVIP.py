...
# Dataset Paths
RAW_DATA_PATH_LLVIP = "datasets_raw/LLVIP/LLVIP"
OUTPUT_PATH_LLVIP = "datasets_preprocess/LLVIP"

# Create output directories
os.makedirs(OUTPUT_PATH_LLVIP, exist_ok=True)

# Function to format Caltech dataset
def process_llvip(split):
    COLOR_FOLDER_NAME = "visible"
    THERMAL_FOLDER_NAME = "infrared"

    split_output_path = os.path.join(OUTPUT_PATH_LLVIP, split)

    color_files = sorted(os.listdir(os.path.join(RAW_DATA_PATH_LLVIP, COLOR_FOLDER_NAME, split)))
    thermal_files = set(os.listdir(os.path.join(RAW_DATA_PATH_LLVIP, THERMAL_FOLDER_NAME, split)))
    ...
    with tqdm(total=num_samples, desc="Formatting LLVIP dataset") as pbar:
        for shard_idx in range(num_shards):
            ...
            with wds.TarWriter(shard_path) as sink:
                for sample_idx in range(shard_idx * shard_size, min((shard_idx + 1) * shard_size, num_samples)):
                    ...
                    try:
                        ir_path = os.path.join(RAW_DATA_PATH_LLVIP, THERMAL_FOLDER_NAME, split, thermal_file)
                        rgb_path = os.path.join(RAW_DATA_PATH_LLVIP, COLOR_FOLDER_NAME, split, color_file)
                    ...
                    pbar.update(1)
    print(f"Total samples formatted for LLVIP: {num_samples}")
